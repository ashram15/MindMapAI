"""Build the binary embedding store by encoding Wikipedia article text 
with Sentence Transformers and writing normalized vectors to `vectors.bin`."""

# This script flattens the 2d matrix of numbers into a 1d binary stream so that the
# C++ code can load it in one single thread command which makes this extremely fast.
...
print(
    f"3. Generating Vectors for {len(texts)} items (this may take a moment)...")
